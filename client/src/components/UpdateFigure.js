...
const UpdateFigure = () => {
    ...
    const updateHandler = (e) => {
        e.preventDefault();
        axios.put(`http://localhost:8000/api/figures/${id}`, {name, season, category, region, rating, image1, image2, image3},
    ...
    const deleteFigure = () =>{
        axios.delete(`http://localhost:8000/api/figures/${id}`,
        {withCredentials:true})
        .then((res)=>{
            console.log(res);
            console.log(res.data);
...
return(
    ...
    <div style ={{height:"900px", backgroundImage: 'url(https://w7.pngwing.com/pngs/998/609/png-transparent-numbers-illustration-technology-binary-file-euclidean-computer-file-binary-background-blue-other-angle.png)' }}>
        ...
        <form onSubmit={updateHandler} class = "form">
            <div>
                <label>Name: </label>
                <input
                    id="name"
            ...
            <button class = "button">Update Figure</button>
            <button class="button" onClick={deleteFigure}>Delete</button>
        </form>
    </div>
)
}
